    ...
    for root, _, files in os.walk(folder_path):
        for file in files:
            if any(file.lower().endswith(ext) for ext in SUPPORTED_EXTENSIONS):
                ...
                media_files.append(f"Media/{relative_path}")
    return media_files

# Write the result to images.json
def write_images_json(folder_path=MEDIA_FOLDER):
    media_files = scan_folder(folder_path)
    output_path = os.path.join(os.path.dirname(__file__), '../static/images.json')
